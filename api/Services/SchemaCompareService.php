...
class SchemaCompareService
{
    ...
    private function generateColumnAlterations(
        ...
        foreach ($toModify as $mod) {
            ...
            if ($driver === 'mysql') {
                ...
                // PostgreSQL: cada atributo requiere su propia sentencia ALTER COLUMN
                if (in_array('type', $diffAttrs, true)) {
                    $pgType = $this->mapTypeForDriver($colType, $driver);
                    $lines[] = "ALTER TABLE {$tbl} ALTER COLUMN {$cn} TYPE {$pgType}" . $sep;
                }
                if (in_array('nullable', $diffAttrs, true)) {
                    $nullAction = (strtoupper($desired['nullable'] ?? 'YES') === 'YES') ? 'DROP NOT NULL' : 'SET NOT NULL';
                    $lines[] = "ALTER TABLE {$tbl} ALTER COLUMN {$cn} {$nullAction}" . $sep;
                }
                if ($needsDefaultAlter) {
                    if ($default !== '') {
                        $defaultVal = trim(str_replace(' DEFAULT ', '', $default));
                        $lines[] = "ALTER TABLE {$tbl} ALTER COLUMN {$cn} SET DEFAULT {$defaultVal}" . $sep;
                    } else {
                        $lines[] = "ALTER TABLE {$tbl} ALTER COLUMN {$cn} DROP DEFAULT" . $sep;
                    }
                }
            } else {
                $colType = $this->mapTypeForDriver($colType, $driver);
    ...
    private function mapTypeForDriver(string $type, string $driver): string
    {
        if ($driver === 'pgsql') {
            ...
            // enum(...) / set(...) de MySQL -> VARCHAR
            if (preg_match('/^enum\(/', $lower) || preg_match('/^set\(/', $lower)) {
                return 'VARCHAR(255)';
            }
            // Enteros MySQL con display width: int(11) -> INTEGER, bigint(20) -> BIGINT
            if (preg_match('/^(tinyint|smallint|mediumint|int|bigint)\(\d+\)$/i', $type, $m)) {
                $base = strtolower($m[1]);
                if ($base === 'tinyint')  return 'SMALLINT';
                if ($base === 'int' || $base === 'mediumint') return 'INTEGER';
                return strtoupper($base);
            }
            // Tipos SQL Server con longitud
            if (preg_match('/^nvarchar\(max\)$/i', $type)) return 'TEXT';
            if (preg_match('/^nvarchar\((\d+)\)$/i', $type, $m)) return "VARCHAR({$m[1]})";
            if (preg_match('/^varbinary\(max\)$/i', $type)) return 'BYTEA';
            ...
            // Tipos nativos de PostgreSQL u otros se devuelven sin cambios
    ...
    private function normalizeDefault(?string $value): ?string
    {
        ...
        // PostgreSQL anexa el tipo con cast: 'abc'::character varying -> 'abc'
        ...
        foreach ($columns as $col) {
            ...
            if ($driver === 'sqlsrv') {
                ...
                if ($extra === 'auto_increment') {
                    // PostgreSQL: columnas auto-incrementales usan SERIAL/BIGSERIAL
                    $serial = (stripos($colType, 'big') !== false) ? 'BIGSERIAL' : 'SERIAL';
                    $lines[] = "    {$cn} {$serial} {$nullable}";
                } else {
                    $colType = $this->mapTypeForDriver($colType, $driver);
                    $lines[] = "    {$cn} {$colType} {$nullable}{$default}";
                }
            } else {
